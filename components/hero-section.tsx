...
export function HeroSection() {
  return (
    <section id="home" className="min-h-screen flex items-center justify-center pt-40 md:pt-24 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="flex flex-col lg:flex-row items-center justify-between gap-8 md:gap-12">
          <div className="flex-1 space-y-6 text-center lg:text-left">
            ...
            <ScrollAnimation animation="scale-in" delay={700} duration={1000}>
            <div className="flex items-center gap-3 sm:gap-4 justify-center lg:justify-start">
              <Button asChild size="icon" variant="outline" className="h-12 w-12 sm:h-14 sm:w-14 rounded-full border-2 hover:scale-110 transition-all duration-300">
                <a href="https://github.com/timi-smiles" target="_blank" rel="noopener noreferrer">
                  <Github className="h-5 w-5 sm:h-6 sm:w-6" style={{ color: '#2563eb' }} />
                </a>
              </Button>
              <Button asChild size="icon" variant="outline" className="h-12 w-12 sm:h-14 sm:w-14 rounded-full border-2 hover:scale-110 transition-all duration-300">
                <a href="https://www.linkedin.com/in/timilehin-ogunnowo" target="_blank" rel="noopener noreferrer">
                  <Linkedin className="h-5 w-5 sm:h-6 sm:w-6" style={{ color: '#0077b5' }} />
                </a>
              </Button>
              <Button asChild size="icon" variant="outline" className="h-12 w-12 sm:h-14 sm:w-14 rounded-full border-2 hover:scale-110 transition-all duration-300">
                <a href="https://x.com/timi_smiles_" target="_blank" rel="noopener noreferrer">
                  <Twitter className="h-5 w-5 sm:h-6 sm:w-6" style={{ color: '#1DA1F2', fill: '#1DA1F2' }} />
                </a>
              </Button>
              <Button asChild size="icon" variant="outline" className="h-12 w-12 sm:h-14 sm:w-14 rounded-full border-2 hover:scale-110 transition-all duration-300">
                <a href="https://wa.me/2348160808720" target="_blank" rel="noopener noreferrer">
                  <svg className="h-5 w-5 sm:h-6 sm:w-6" viewBox="0 0 24 24" fill="#25D366" xmlns="http://www.w3.org/2000/svg">
          ...
          <ScrollAnimation animation="fade-left" delay={600} duration={1000}>
          <div className="flex-shrink-0 w-full flex justify-center lg:block mt-8 lg:mt-0">
            <div className="relative">
              <div 
                className="absolute inset-[-0.75rem] md:inset-[-1rem] bg-primary/5 blur-[3xl] md:blur-[4xl] animate-pulse scale-110"
                style={{ borderRadius: '60% 40% 60% 40%' }}
              />
              <div 
                className="relative w-64 h-[22rem] sm:w-80 sm:h-[26rem] lg:w-[28rem] lg:h-[36rem] shadow-2xl overflow-hidden p-1 hover:scale-105 transition-transform duration-500"
                style={{ 
